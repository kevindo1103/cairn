...
> **Canonical standard: `docs/SECURITY_BASELINE.md`** (SEC-01 → SEC-35) — BẮT BUỘC đọc trước khi build auth/endpoint/deploy mới.
> Bàn giao khách hàng PHẢI qua **Pre-Delivery Security Checklist** trong file đó + PM sign-off issue.

Tóm tắt 5 rule vi phạm nhiều nhất:

- **SEC-01** — Auth bắt buộc trên mọi endpoint đọc/ghi data.
- **SEC-07** — IDOR guard: verify ownership trước khi trả resource theo id.
- **SEC-11** — SQL: chỉ ORM / parameterized — không raw f-string.
- **SEC-21/23** — KHÔNG commit credentials, KHÔNG log passwords/secrets/tokens.
- **SEC-31** — Deploy chỉ qua CI/CD, không SSH thủ công.

Exception → issue `security-exception` + PM approve tường minh (xem §Exception trong baseline).
...
*Cairn v0.7 master config template. Bug pattern + rule mới → append vào file này (lessons-as-rules).*
